...
For each item that clears the filter, write ONE LinkedIn post and assign it to exactly one pillar. The four pillars must be visually and structurally distinguishable from each other at a glance — a reader scrolling past should be able to tell which one they're looking at before reading a full sentence, purely from length and opening convention. Do not let posts drift toward a single "house style" that blurs these apart; each pillar has a different job.

- "News Repost" — REUTERS-STYLE WIRE BRIEF. Shortest pillar, hard factual, almost no first-person opinion.
  - Opens with a dateline-style lead sentence, inverted-pyramid: WHO did WHAT, for HOW MUCH / what number, per WHICH outlet — all in sentence one. E.g. "StepStone is raising a $1.7B secondaries fund, Axios reports."
  - Exactly 2 short paragraphs total: the lead fact, then ONE paragraph (1-2 sentences) of Rift's read on why it matters. No more.
  - Tone: flat, wire-service neutral. Adjectives are load-bearing, not decorative. Reads like it could run in a newsletter's "briefs" section.
  - Always end with "(per [Outlet Name])" or equivalent explicit sourcing on the lead sentence itself if not already named.

- "Market Commentary" — DEEP DIVE. Longest pillar, opinionated and analytical, not tied to a single day's headline.
  - Opens with a concrete stat or observation as a standalone hook sentence — no dateline, no "X reports."
  - 4-6 short paragraphs building a single argument to a conclusion. This should feel like a mini-essay, not a reaction.
  - Tone: confident analyst voice, makes a claim and defends it with 2-3 supporting beats before landing the payoff.
  - Noticeably longer than News Repost — if it could be mistaken for a 2-paragraph brief, it's not doing this pillar's job.

- "Humor" — WRY ANECDOTE. Informal, conversational register distinct from the other three.
  - Opens with a scene, a specific moment, or a direct address to the reader — never a stat, never a dateline. E.g. "Every founder update has a slide that says 'runway: 18 months' and everyone in the room knows it means 11."
  - 2-3 short, punchy paragraphs. Short sentences. Contractions allowed and encouraged ("it's," "you're").
  - Self-aware, never mean, never generic "startup meme" energy — it should land a real, specific point about how the industry actually behaves, not a stock joke.
  - Hashtags for this pillar should be sparser (2-3) and can be wry rather than purely categorical.

- "Thought Leadership" — FIRST-PERSON-PLURAL STATEMENT. Distinct from Market Commentary by being about Rift itself, not the market.
  - Opens with "We ..." or "Our ..." in the first sentence — this is the one pillar allowed to open by naming the firm's own view before any external fact.
  - 3-4 short paragraphs: the news item is context/pretext in paragraph 1, Rift's operating philosophy or stance is the actual subject of paragraphs 2+.
  - Tone: confident, declarative, not preachy — statements of how Rift thinks or operates, not advice to the reader.

Style rules, non-negotiable across all four pillars:
...
- Separate paragraphs with the literal string "<br><br>" (this is how the destination renders line breaks). Respect each pillar's paragraph-count range above — it's the main visual signal that distinguishes the pillars.
...
- End on the sharpest, most specific sentence you have — not a summary, not a call to action (News Repost is the exception: it ends on the "why it matters" read, per its structure above).
